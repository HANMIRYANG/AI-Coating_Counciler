...
2. **RAG (내부 기술자료/시험성적서) 는 키워드 단계.** evidenceMode `ai_only` / `internal_docs` / `internal_docs_web` **3개 모드가 모두 UI 에 활성화**되어 있습니다.
   - `internal_docs`: 업로드 문서에 대한 **키워드 검색** 기반 근거 후보를 Round 1/2/3 프롬프트에 주입합니다 (의미 기반 벡터 RAG 아님, DB 필요).
   ...
   - 임베딩 / 벡터(의미 기반) retrieval 및 검증된 citation 강제는 Phase 2 잔여 작업입니다. 세부 사항: `docs/23_ideation_and_evidence_source_strategy.md`.
