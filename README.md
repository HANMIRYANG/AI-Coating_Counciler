...
2. **RAG (내부 기술자료/시험성적서) — 의미 기반(하이브리드) 검색 단계.** evidenceMode `ai_only` / `internal_docs` / `internal_docs_web` **3개 모드가 모두 UI 에 활성화**되어 있습니다.
   - `internal_docs`: 업로드 문서에 대한 **키워드 + 벡터(임베딩) 하이브리드 검색** 기반 근거 후보를 Round 1/2/3 프롬프트에 주입합니다 (`EVIDENCE_RETRIEVAL_MODE`=hybrid 기본, DB 필요). 임베딩은 `OpenAI text-embedding-3-small`, `USE_MOCK_PROVIDERS=true`/키 없음이면 결정적 MockEmbedder. 저장은 앱레벨 코사인(`DocumentChunk.embedding`, 마이그레이션 없음). 기존 청크는 `POST /api/documents/embeddings/backfill` 로 임베딩.
   - `internal_docs_web`: 위에 더해 **사용자 제공 공식 URL** 을 서버에서 fetch 합니다 (`apps/web/src/lib/council/sourceFetch.ts`).
   - 외부 공식 출처 (KOLAS/KATS, KCL, KTR, KTC, FITI, KATRI, KOTITI, KFI, KICT 등) **카탈로그 기반 자동 조회**는 아직 구현되지 않았습니다 (`GET /api/evidence-sources` 의 `retrievalEnabled=false`).
   - **pgvector 인덱스**(대규모 확장), **검증된 citation 강제 / Retrieval Guard 게이팅**은 다음 슬라이스입니다. 세부 사항: `docs/13_rag_document_strategy.md`.
