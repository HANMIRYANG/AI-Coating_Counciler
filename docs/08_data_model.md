...
| 인용 evidence (Phase 2) | — | `Document`, `DocumentChunk` (업로드/추출/임베딩 사용 중) |
...
가 저장 전에 제거하며, `GET /api/documents` summary 노출 + 검색 metadata
필터에 사용됩니다. `DocumentChunk.embedding` 은 청크 임베딩(LE Float32)을
저장하며 앱레벨 코사인 기반 벡터/하이브리드 검색에 사용됩니다(인텍이크 시
best-effort, 기존 청크는 `POST /api/documents/embeddings/backfill` 로 채움).
