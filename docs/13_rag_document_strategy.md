...
>
> - **검증된 인용 렌더링 (Verified Citations)** (`lib/council/verifiedCitations.ts` + sessionMarkdown "검증된 인용" 섹션 + FinalEvidenceCoveragePanel/view-model): **이미 검증된** evidence usage 계약(`evidenceUsed`/`coveredClaims`/`uncoveredClaims`/`retrievalGuard`)을 결정적으로 재표현한다. covered claim 에 `[C1]`/`[C2]` 라벨, 인용 근거에 `[E1]`/`[E2]` 라벨(filename#chunkIndex · 신뢰수준 · 검증상태)을 부여하고, uncovered 는 미연결로 분리한다. `citationReady`는 ① 가드 verdict 가 **`guardStatus="passed"` 이고 `businessCitationReady=true`**(둘 다 요구 — 모순 payload 방어)이며 ② 모든 cited claim 이 ≥1 evidenceUsed ref 로 해석되고 ③ 미연결(uncovered) 주장이 0건일 때만 true. **모델 호출 없음**, raw chunk 본문/내부 chunkId 비노출, 결정적 dedupe·정렬. 표시/내보내기 전용.
>
> - **인용 무결성 점검 (Citation Integrity)** (`lib/council/citationIntegrity.ts` + sessionMarkdown "인용 무결성 점검"·"근거 부록" 섹션 + FinalEvidenceCoveragePanel/view-model): 검증된 인용 위에서 검토/내보내기 준비 상태를 결정적으로 분류한다. `integrityStatus` `ready|review_required|blocked` + `reviewRequired`/`exportReady`/`summary`/`recommendations`. **이슈는 심각도로 분류**된다 — `problem`(준비 관련: `unresolved_claim`/`missing_evidence_ref`/`not_business_ready_guard`/`no_cited_claims`/`unguarded_legacy_answer`) 와 `advisory`(인라인 라벨: `body_has_no_citation_labels`/`body_missing_citation_labels`/`body_has_unknown_citation_labels`). 결과는 `problemIssues`/`advisoryIssues`/`problemCount`/`advisoryCount` 및 분리 권장(`problemRecommendations`/`advisoryRecommendations`)을 제공하고, `summary` 는 자문-only 결과를 "문제"로 표기하지 않는다(ready+자문 → "양호 … (자문 N건)"). `ready`는 citationReady 충족 시, `blocked`는 가드 blocked 또는 (근거 필수 ∧ citationReady=false) 시, 그 외 `review_required`. **모순 가드 방어**: `businessCitationReady=true` 라도 `guardStatus≠passed`(blocked/warning)이면 not business-ready 로 취급 → ready 가 되지 않고 `not_business_ready_guard` problem 을 보고한다. **`not_required` 가드 + cited/unresolved 주장 0건(일반 ai_only/저위험)이면 무결성·부록 섹션을 표시하지 않는다**(`hasCitationIntegrityMaterial`; blocked 가드는 클레임 0건이어도 표시). 본문 인용 라벨 점검(`businessReadyAnswer`+`finalMarkdown` 의 `[C#]` 을 생성 라벨과 비교: 없음/누락/미지정)은 **자문 전용 — `integrityStatus`/`reviewRequired`/`exportReady`/`citationReady` 를 절대 강등하지 않는다**(현재 인용은 본문이 아닌 별도 목록으로 표시). export 와 UI 는 문제와 자문을 분리 표시한다. 검증된 인용 claim 행은 근거를 `[E1] filename#chunkIndex` 형태로 표기해 **근거 부록의 `[E#]` 와 직접 연결**된다. export 에는 deduped **근거 부록**(`[E#]`·filename#chunkIndex·신뢰수준·검증상태) 포함. **모델 호출 없음**, raw chunk 본문/내부 chunkId 비노출.
>
> 미구현 (범위 밖):
> - **pgvector 인덱스**: 현재는 bounded 스캔 + 인프로세스 코사인. 대규모 코퍼스 확장 시 pgvector 로.
> - **법적/사실 인증이 아님**: Retrieval Guard(인용 충분성 게이트) · Verified Citations(주장→근거 결정적 렌더링) · Citation Integrity(검토/내보내기 준비 점검)는 모두 **이미 검증된 매핑의 결정적 표현/분류**일 뿐이며, 사실의 법적 인증이나 자동 사실검증이 아니다. 모델 prose 가 특정 chunk 를 강제 인용하게 만드는 grounding 은 여전히 향후 작업.
